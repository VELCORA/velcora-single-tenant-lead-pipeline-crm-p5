  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand"><img src="/assets/3_d_model_is_here.png" alt="VeloCRM logo" /><div><strong>velocrm</strong><span>Lead operations</span></div></div>
...
function Intake({ onCreated, compact = false }: { onCreated: (id: string) => Promise<void>; compact?: boolean }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    const score = Math.min(98, 42 + (form.company ? 18 : 0) + (form.role ? 12 : 0) + (form.message.length > 40 ? 16 : 0) + (form.phone ? 10 : 0));
    const priority: Priority = score >= 75 ? 'hot' : score >= 55 ? 'warm' : 'cold';
    const { data, error: insertError } = await supabase.from('leads').insert({ ...form, score, priority, email_status: 'queued', next_follow_up_at: new Date(Date.now() + 86400000).toISOString() }).select('id').maybeSingle();
    ...
    await supabase.from('lead_activities').insert({ lead_id: data.id, type: 'created', content: `Lead captured from ${form.source}` });
    await supabase.from('lead_activities').insert({ lead_id: data.id, type: 'qualification', content: `Scored ${score}/100 · ${priority} priority` });
    await supabase.from('notifications').insert({ lead_id: data.id, type: 'new_lead', title: 'New lead captured', content: `${form.name} from ${form.company} is ready for review.` });
